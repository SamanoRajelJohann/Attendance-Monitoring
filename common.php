...
function handleFormSubmission($conn){
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($action == "insert") {
            $sql = "INSERT INTO student (ID, First_Name, Middle_Name, Last_Name, BirthDate, Address) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssss", $ID, $First_Name, $Middle_Name, $Last_Name, $BirthDate, $Address);
            $stmt->execute();
            $stmt->close();
...
function displayStudentData($conn){
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "</td>";
            echo "<td>
                    <form action='updatestudentdata.php' method='get'>
                        <input type='hidden' name='id' value='" . $row["ID"] . "'>
                        <input type='hidden' name='fn' value='" . $row["First_Name"] . "'>
                        <input type='hidden' name='mnd' value='" . $row["Middle_Name"] . "'>
                        <input type='hidden' name='ln' value='" . $row["Last_Name"] . "'>
                        <input type='hidden' name='bd' value='" . $row["BirthDate"] . "'>
                        <input type='hidden' name='ar' value='" . $row["Address"] . "'>
...
function displayTeacherData($conn) {
...
function login($conn){
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Validate the action and handle accordingly
        if ($action == "insert") {
            $sql = "INSERT INTO accountsignup (UserName, Email, Password)
